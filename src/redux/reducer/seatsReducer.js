...
// const seatsStorage = JSON.parse(localStorage.getItem(STORAGE.SEATS))

const initialState = {
    // seats: seatsStorage ? [...seatsStorage] : [...seats],
    seats: [...seats],
    seatsSelected: []
}
...
export const seatsReducer = (state = initialState, action) => {
    ...
    switch (type) {
        case SELECT_SEAT: {
            let indexSeatOfRow = 0
            const indexRow = state.seats.findIndex((seatData) => {
                let rowSelecting = {}
                indexSeatOfRow = seatData.danhSachGhe.findIndex((ghe) => ghe.soGhe === payload.soGhe)
                if (indexSeatOfRow !== -1) rowSelecting = seatData
                return seatData.hang === rowSelecting.hang
            })

            const newSeats = [...state.seats]
            newSeats[indexRow].danhSachGhe[indexSeatOfRow].selecting = newSeats[indexRow].danhSachGhe[indexSeatOfRow].selecting ? false : true
            return { ...state, seats: newSeats }
        }
        case SAVE_SEATS_SELECTED: {
            const newSeatsSelected = state.seats.map(seatData => ({ ...seatData, danhSachGhe: seatData.danhSachGhe.filter(seat => seat.selecting) })).filter(seatData => seatData.danhSachGhe.length)
            const newSeats = state.seats.map(seatData => ({ ...seatData, danhSachGhe: seatData.danhSachGhe.map(seat => ({ ...seat, daDat: Boolean(seat.selecting) })) }))

            localStorage.setItem(STORAGE.SEATS, JSON.stringify(newSeats))
            return { ...state, seatsSelected: newSeatsSelected, seats: newSeats }
        }
        default:
            return state
    }
